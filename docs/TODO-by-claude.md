...
- [ ] `localhost:3000` → 관리자 UI 전혀 없음 확인 (수동 테스트)
- [ ] `localhost:3000/?admin` → 비밀번호 모달 표시 확인 (수동 테스트)
- [ ] 비밀번호 입력 → AdminBar 표시, 각 섹션 편집 버튼 표시 확인 (수동 테스트)
- [ ] 프로젝트 ✏️ → 폼 모달 → 저장 → 카드 즉시 업데이트 (수동 테스트)
- [ ] 프로젝트 🗑️ → 카드 삭제 (수동 테스트)
- [ ] [+ 프로젝트 추가] → 카드 추가 (수동 테스트)
- [ ] 페이지 새로고침 (`?admin` 유지) → 변경사항 localStorage에서 복원 (수동 테스트)
- [ ] "내보내기" → 코드 복사 확인 (수동 테스트)
- [ ] "초기화" → 원본 데이터 복원 (수동 테스트)
- [ ] Phase 모달 → 레이어 추가/수정/삭제 (수동 테스트)
